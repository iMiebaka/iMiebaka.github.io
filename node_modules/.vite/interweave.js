...
function createConfigBuilder(config) {
  return (tagName) => {
    tagConfigs[tagName] = __spreadValues(__spreadValues({}, config), tagConfigs[tagName]);
  };
}
...
var Parser = class {
      ...
      while (matchedString && (parts = matcher.match(matchedString))) {
        ...
        if (valid) {
          ...
          elements[tokenName] = {
            children: match2,
            matcher,
            props: __spreadProps(__spreadValues(__spreadValues({}, props), partProps), {
              key: this.keyIndex
            })
          };
        } else {
  ...
  getTagConfig(tagName) {
    ...
    if (TAGS[tagName]) {
      return __spreadProps(__spreadValues(__spreadValues({}, common), TAGS[tagName]), {
        tagName
      });
    }
    return common;
    ...
    [...parentNode.childNodes].forEach((node) => {
      if (node.nodeType === ELEMENT_NODE) {
        ...
        if (!(noHtml || noHtmlExceptMatchers && tagName !== "br") && this.isTagAllowed(tagName) && (allowElements || this.canRenderChild(parentConfig, config))) {
          ...
          content.push(import_react2.default.createElement(Element, __spreadProps(__spreadValues({}, elementProps), {
            key: this.keyIndex
